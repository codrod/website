...
class Config {
  ...
  _mergeConfigObj(config, element) {
    const jsonConfig = util_isElement(element) ? manipulator.getDataAttribute(element, 'config') : {} // try to parse

    return {
      ...this.constructor.Default,
      ...(typeof jsonConfig === 'object' ? jsonConfig : {}),
      ...(util_isElement(element) ? manipulator.getDataAttributes(element) : {}),
  ...
  _typeCheckConfig(config, configTypes = this.constructor.DefaultType) {
    for (const [property, expectedTypes] of Object.entries(configTypes)) {
      ...
      const valueType = util_isElement(value) ? 'element' : toType(value)
...
class Dropdown extends base_component {
  ...
  show() {
    ...
    const showEvent = event_handler.trigger(this._element, EVENT_SHOW, relatedTarget)
    ...
    event_handler.trigger(this._element, EVENT_SHOWN, relatedTarget)
  }
  ...
  _completeHide(relatedTarget) {
    const hideEvent = event_handler.trigger(this._element, EVENT_HIDE, relatedTarget)
    ...
    event_handler.trigger(this._element, EVENT_HIDDEN, relatedTarget)
  }

  _getConfig(config) {
    config = super._getConfig(config)

    if (typeof config.reference === 'object' && !util_isElement(config.reference) &&
  ...
  _createPopper() {
    ...
    } else if (util_isElement(this._config.reference)) {
  ...
  static dataApiKeydownHandler(event) {
    ...
    const getToggleButton = this.matches(SELECTOR_DATA_TOGGLE) ?
      this :
      (selector_engine.prev(this, SELECTOR_DATA_TOGGLE)[0] ||
        selector_engine.next(this, SELECTOR_DATA_TOGGLE)[0] ||
        selector_engine.findOne(SELECTOR_DATA_TOGGLE, event.delegateTarget.parentNode))
...
event_handler.on(document, EVENT_KEYDOWN_DATA_API, SELECTOR_DATA_TOGGLE, Dropdown.dataApiKeydownHandler)
event_handler.on(document, EVENT_KEYDOWN_DATA_API, SELECTOR_MENU, Dropdown.dataApiKeydownHandler)
event_handler.on(document, EVENT_CLICK_DATA_API, Dropdown.clearMenus)
...
const collapse_CLASS_NAME_SHOW = 'show'
...
class Collapse extends base_component {
  constructor(element, config) {
    ...
    for (const elem of toggleList) {
      const selector = selector_engine.getSelectorFromElement(elem)
  ...
  show() {
    ...
    const startEvent = event_handler.trigger(this._element, collapse_EVENT_SHOW)
    ...
    const complete = () => {
      ...
      event_handler.trigger(this._element, collapse_EVENT_SHOWN)
    }
  ...
  hide() {
    ...
    const startEvent = event_handler.trigger(this._element, collapse_EVENT_HIDE)
    if (startEvent.defaultPrevented) {
      return
    }
    ...
    for (const trigger of this._triggerArray) {
      const element = selector_engine.getElementFromSelector(trigger)
    ...
    const complete = () => {
      ...
      event_handler.trigger(this._element, collapse_EVENT_HIDDEN)
    }
  ...
  _initializeChildren() {
    ...
    for (const element of children) {
      const selected = selector_engine.getElementFromSelector(element)
...
event_handler.on(document, collapse_EVENT_CLICK_DATA_API, collapse_SELECTOR_DATA_TOGGLE, function (event) {
  ...
  for (const element of selector_engine.getMultipleElementsFromSelector(this)) {
...
//Note looks the treeshaking logic will include all bootstrap modules unless we import from specific files.
